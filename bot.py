...
inputcondition = ''
conditionfile = file("keywords.txt","r")
tempcondlist = []
while True:
  cond = conditionfile.readline()
  if cond is '':
    break
  cond = cond.rstrip('\n')
  cond = lower(cond)
  tempcondlist.append(cond)
conditionfile.close()
#tempcondlist now contains the possible conditions in the input

condlist = []
for i in tempcondlist:
  i = i.split('/')
  condlist.append(i)
...
while True :
  ...
  # We check if a condition has been mentioned in the input
  condition = ''
  for i in condlist:
    if i in input:
      condition = i
